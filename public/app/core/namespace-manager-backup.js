...
class NamespaceManager {
  ...
  addToNamespace(namespacePath, moduleName, moduleValue, options = {}) {
    ...
    // 使用日志系统
    const logger = window.loggers?.architecture || console;
    logger.debug?.(`添加模块到命名空间: ${namespacePath}.${moduleName}`) || (typeof isDevelopment !== 'undefined' && isDevelopment && console.log(`📦 添加模块到命名空间: ${namespacePath}.${moduleName}`));
    return true;
  }
      ...
      try {
        ...
        currentGlobals.forEach(propertyStr => {
          ...
          if (!existingGlobals.has(propertyStr) && 
            ...
            console.warn(`⚠️ 检测到新的全局变量: ${propertyStr}，建议使用命名空间`);
